...
public class NoiseTexture : Texture
{
    ...
    public override Vector3D Value(float u, float v, Vector3D p)
    {
        //return new Vector3D(1, 1, 1) * 0.5f * (1 + Perlin.Turb(scale* p));
        //return new Vector3D(1, 1, 1) * Perlin.Turb(scale * p);
        return new Vector3D(1, 1, 1) * 0.5f*(1+(Mathf.Sin(scale*p.Z+ 10* Perlin.Turb(p))));
    }
}
