...
{
    /// <summary>
    /// 本项目采用多个线程同时渲染，逐步增加采样率的方式进行渲染，同时使用了层次包围盒(BVH树)来管理场景中的物体，
    /// 轴对齐包围盒(AABB)来处理物体的碰撞检测,并优化了用于生成随机数的随机函数，使用directX11的窗口来展示结果，大大加快了渲染速度
    /// 
    /// 本节展示的这张图大家都是渲染过的，这是一张2000*1000的图，没有经过优化的话渲染是要花十几个小时的。
